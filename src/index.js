// we want .env to load all variable to each files as early as possible so configuring dotenv must be in main file
 ...
 connectDB()
 .then(()=>{
    //before app.listen we have listen for error also (app.on)
    app.on("ERRR:" , (error)=>{
        console.log("ERRR:",error);
        throw error;
    })
    //we use callback , because callback only run/happen when , app.listen(event) or say server(async) starts/happens 
